...
export class FooterButtonsComponent implements OnChanges {
  ...
  ngOnChanges() {
    if (this.completedQuestionCount > 0) {
      this.percentage = this.questionCount
        ? (this.completedQuestionCount / this.questionCount) * 100
        : 0;
    ...
    if (this.updatedData.responseType.toLowerCase() === "slider") {
      if (
        ...
        JSON.stringify(this._data.value) !==
          JSON.stringify(this.updatedData.value)
      ) {
      ...
      this.updatedData.responseType.toLowerCase() === "pagequestions"
    ) {
      if (
        JSON.stringify(this._data.pageQuestions) !==
          JSON.stringify(this.updatedData.pageQuestions) &&
