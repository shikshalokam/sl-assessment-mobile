...
export class ResetPasswordPage {
  ...
  encryptParams(stringToEncrypt) {
    ...
    this.http.post(AppConfigs.punjabBaseUrl + AppConfigs.punjab.encryptedMethod, params, httpOptions).subscribe(
      (success) => {},
      (error) => {
  ...
  resetPassword() {
    ...
    this.http.post(AppConfigs.punjabBaseUrl + AppConfigs.punjab.resetPassword, params, httpOptions).subscribe(
      (success) => {},
      (error) => {
        ...
        try {
          ...
          if (
            xmlDoc
              ...
              .childNodes[0].nodeValue.indexOf("Password & confirm password is not same !!!") >= 0
          ) {
            this.utils.openToast("Password & confirm password is not same !!!");
          } else if (
            xmlDoc.getElementsByTagName("string")[0].childNodes[0].nodeValue.indexOf("Your password has chanced !!!") >=
            0
          ) {
